...
- **Never wait silently for more than 1 hour.** The prompt cache TTL is 1 hour, so a single silent gap longer than that invalidates the whole cached session context and the next wake-up pays a full re-read. Any waiter or monitor watching something that can run past an hour (CI, a cold build, an external queue) must produce an event within ~50 minutes: give the waiter a ~50-minute timeout and re-arm it on the timeout notification, or build a heartbeat into the monitor loop (one "still running" line every ~45 minutes). This complements the no-polling rule rather than contradicting it — a heartbeat comes from the armed waiter itself, not from manual progress checks. The rule also covers the end of a turn: never end a turn with substantive session work still pending unless something armed will provably produce an event within ~50 minutes — the failure mode is that every waiter fires, the turn ends, the user is away, and the idle session silently outlives the cache. Keep one persistent session heartbeat monitor armed across long multi-turn work, and stop it only when the session's work is genuinely finished. Waiters are also litter: kill every waiter or monitor the moment its purpose is served or superseded — a panel full of hours-old zombie waits is a bug in your process.
...
- MANDATORY for every agent (Claude Code, Codex, and any future agent): **the canonical checkout is read-only.** Investigate in it freely; make every edit inside an **agent workspace**. There is no size below which a direct edit is acceptable, because size is not what the cost scales with: several sessions share this one checkout, and `finish_workspace.sh` refuses to run while it has uncommitted changes, so a one-line edit left in the tree blocks every other agent from merging exactly as a thousand-line one does. "Small enough" is self-assessed and always feels true. If it produces a diff, it belongs in a workspace — and a workspace on the same volume as the checkout takes seconds to create. The tooling is committed in-repo at `.claude/skills/waterui-agent-workspace/` (see its `SKILL.md`) so it travels with the repository and is identical for all agents — do not depend on any agent-private copy. The lifecycle, run from the repository root:
  - create: `.claude/skills/waterui-agent-workspace/scripts/create_workspace.sh <task-slug>` (reuses a FREE workspace *slot* at a fixed path when one exists, fast-forwarding it onto canonical instead of cloning; only clones a brand new slot, COW-copying `target/`, when every slot is busy — either way it branches everything `agent/<slug>/<timestamp>`). Then `cd` into the printed workspace and do all edits/builds/tests there. Reuse is fast because the slot's path never changes, so Cargo's `-C metadata` and sccache's cache key stay warm across tasks instead of recompiling the whole workspace; only first-time seeding of a brand new slot pays the slow, quiet `target/` copy — long periods without output during that one-time seed are not suspicious by themselves.
  - sync (when canonical advances): `.claude/skills/waterui-agent-workspace/scripts/sync_workspace.sh` from inside the workspace.
  - finish (merge back): `.claude/skills/waterui-agent-workspace/scripts/finish_workspace.sh` from inside the workspace — it takes the canonical integration lock, fast-forwards each submodule then the superproject onto their configured branches, and releases the slot (switches it back onto its integration branches, fast-forwards, deletes the spent agent branch) so the next task reuses it warm; legacy timestamped workspaces predating the slot model are still deleted. `finish_workspace` is a SCRIPT, not a built-in tool or agent command: invoke it, never hand-roll the fast-forward (a manual merge silently skips the submodule back-merge and the integration lock). All defaults are derived from git/`$HOME`; override only via `WATERUI_AGENT_SOURCE_REPO` / `WATERUI_AGENT_WORKSPACE_ROOT` / `WATERUI_AGENT_LOCK_ROOT` when a machine's layout differs.
...
- The scope of a frame is all-or-nothing; *whether* to run one is a separate question, and Hydrolysis is free not to. The window pump is a three-state machine (`FrameMode` in `runner/window.rs`): `Idle` does no work at all, `Reencode` re-samples visual state and re-encodes the whole retained tree without re-running layout, and `Refresh` relayouts the retained tree and re-encodes it. A window with nothing to show does not burn a frame, and a visual-only change (an animation tick, a scroll offset) does not pay for layout. This is not damage tracking — every frame that *does* run still redraws the whole scene. Do not conflate the two: adding partial-region painting is forbidden, while skipping an idle frame is the design.
