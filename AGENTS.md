...
- For filter visual review images, the canonical reusable infrastructure is `cargo test -p waterui-graphics --lib filter_view::tests::gpu_export_filter_gallery_images -- --nocapture`, which exports PNG files to `/tmp/waterui_filter_gallery/`. Use this path to show filter outputs instead of creating a new gallery generator.
