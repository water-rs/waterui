...
- **Experimental backends never gate a release.** `waterui-gtk` (water-rs/gtk-backend), `waterui-winui` and the TUI backend are experimental: their nightlies are informational, like bench legs. A red leg in one of them during a release — a failing example, a blank capture, a broken golden — becomes an issue in that backend's repository carrying the run id and the evidence, and the release proceeds without waiting for it. The release contract is the Apple and Android backends, the CLI, and Hydrolysis; only their nightlies and the framework's own checks are prerequisites for a `dev` → `main` merge. Experimental backends still ship in the satellite wave when their manifests are ready, but never hold the framework or the other satellites back, and nobody dispatches control runs or debugs them on the release's critical path.
