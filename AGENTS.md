...
- **Hydrolysis and its Material 3 theme are two independent repositories.** `hydrolysis-m3` lives in water-rs/hydrolysis-m3 and is no longer in this tree; extract `backends/hydrolysis` into its own repository too, and do not nest the MD3 package in the renderer repository or retain either as a WaterUI submodule. Each owns its implementation, dedicated examples, assets, tests, CI, and releases. Hydrolysis remains theme-independent and GPU-required; the MD3 package implements public widget/theme contracts without depending on renderer internals. WaterUI's CLI and integration tests select explicit compatible versions of both packages through channel metadata.
...
The Material 3 widget theme for the Hydrolysis renderer is not in this
directory: `hydrolysis-m3` lives in water-rs/hydrolysis-m3 (#481) and is
consumed from crates.io. The version the workspace builds against is the
`[workspace.dependencies]` entry in the root manifest, and
`[patch.crates-io]` there resolves the framework crates it names to this
