...
- Do not update this repo's `.claude/skills/waterui/SKILL.md` as part of ordinary implementation, conformance, testing, visual-acceptance, or maintainer workflow work. The repo-local skill is for WaterUI users, not for tracking agent discoveries or implementation progress.
...
- Keep `.claude/skills/waterui/SKILL.md` strictly user-facing: view authoring, reactivity, components, app-level CLI usage, and preview usage. Do not put maintainer-only workflow rules, test/benchmark execution policy, CI/package matrices, release metadata, submodule handling, proxy/SDK/NDK repair notes, linker workarounds, agent/subagent mechanics, visual-acceptance results, Material token audit notes, component conformance checklists, or other Codex/Claude behavior rules in the skill. If a rule tells agents how to work, verify, skip, debug, or maintain the repository, it belongs in `AGENTS.md`, not in the user-facing skill.
- Material Design or backend component implementation details are not automatically user-facing. Do not add per-component Material token values, shape metrics, animation internals, or conformance findings to `.claude/skills/waterui/SKILL.md` unless the user explicitly asks to document that public authoring API for WaterUI users.
