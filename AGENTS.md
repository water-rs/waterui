...
One exemption: the repository owner @lexoliu personally waives this review gate on repositories he owns (his global rules override this section). Sessions working under his authority do not pause for review — land the standard topic-branch → PR-to-`dev` flow directly. The gate still applies to every other operator.
...
- **This repository is the non-self-drawn core, and nothing else.** What stays here is the part of WaterUI that needs every native backend to cooperate: `waterui-core` and the reactive, layout and text foundations, the FFI contract and the Apple/Android/GTK bridges, `waterui-backend-core`, the CLI, and the components whose realization is a native primitive on each platform. Everything self-drawn — the renderers (`hydrolysis`, `hydrolysis_m3`, `waterui-dew`) and every component that paints its own pixels (`waterui-svg`, `waterui-math`, `waterui-mermaid`, the crates listed in the next bullet) — lives in its own repository under `water-rs` and iterates on its own schedule. A split repository depends on the framework as published on crates.io, never on a git branch of this tree, and this tree consumes it from crates.io in turn, never through a git pin or a path patch. The consequence is a fixed order for any change that crosses the boundary: the framework change lands here and ships in a framework release first, then the split repository adopts it in its own pull request; a framework pull request never carries split-repository code, and a split repository never blocks one. A new self-drawn component starts as its own repository, created from the `hydrolysis` template, and is never added to this tree.
...
- **The canonical channel for a proc macro to convey metadata to the `water` CLI (or any tooling) is metadata symbols in the compiled artifact.** The macro emits a `#[used] static` whose item name starts with `waterui_meta_` and whose bytes carry the payload. `#[used]` — never `#[no_mangle]`/`#[export_name]` — is the load-bearing choice: it keeps the item in the object file and rlib while leaving the final link free to garbage-collect it, and it never enters a cdylib's export table, so the shipped binary carries zero bytes of it. The name is mangled but the mangling contains the item path, so tooling enumerates with `object` plus `rustc-demangle` and prefix-matches. Read it from the rlib the user crate is compiled into — the CLI-generated FFI/bin wrapper is the leaf crate, so the user crate is always an rlib in `deps/`; a bin crate has no intermediate to read, which is a constraint on project shape, not an implementation detail. Metadata consumed only by development-loop features (preview discovery, dev-server handoff) additionally goes behind `#[cfg(debug_assertions)]`; metadata consumed by packaging (bundle mounts, a staged web frontend) is emitted in every profile. The artifact is ground truth that has already resolved macros, cfgs, and generics, so indirection that defeats source scanning cannot defeat this. Do not invent per-feature side channels — generated files, env hacks, source scraping; extend this one mechanism instead. Do not run external processes (bundlers, generators) inside a proc macro either: rust-analyzer re-expands macros on every edit, `cargo check`/`clippy` expand them too, and packaging builds several targets — expansion must stay a pure read; anything that runs a tool belongs to the CLI after the build.
...
- A published CLI scaffolds projects against `[package.metadata.waterui-scaffold]` in `cli/Cargo.toml` plus the values `cli/build.rs` derives: every workspace crate's version (a released CLI pins them at its own package version, since the release wave publishes them together) and each backend's git ref (the submodule commit in a checkout, the `v<version>` tag the release workflow pushes to the backend repository). The table itself keeps only what no manifest supplies — extracted-crate requirements like `hydrolysis-version`, `android-kotlin-version`, and the `*-backend-url` coordinates — and `build.rs` fails the build with the exact lines to write whenever they drift from the workspace. That is the point: a CLI released with stale literals hands every new project a framework a release or two old (#548).
