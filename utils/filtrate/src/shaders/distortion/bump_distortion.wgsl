...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) { let dims=vec2<u32>(uniforms.output_dimensions); if gid.x>=dims.x||gid.y>=dims.y{return;} let coord=vec2<i32>(gid.xy); let uv=(vec2<f32>(gid.xy)+vec2<f32>(0.5))/uniforms.output_dimensions; let center=vec2<f32>(param(0u),param(1u)); let radius=max(param(2u), 0.001); let scale=param(3u); let delta=uv-center; let dist=length(delta); var sample_uv=uv; if dist<radius { let t=1.0 - dist/radius; let factor=1.0 + scale * t * t; sample_uv = center + delta / factor; } let input_dims=vec2<i32>(vec2<u32>(uniforms.input_dimensions)); let mapped=clamp(vec2<i32>(sample_uv*uniforms.input_dimensions), vec2<i32>(0), input_dims-vec2<i32>(1)); textureStore(output_texture, coord, textureLoad(input_texture, mapped, 0)); }
