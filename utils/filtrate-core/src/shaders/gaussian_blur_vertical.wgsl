// Separable vertical gaussian blur pass.
...
fn main(
    ...
    if input_dims_u == dims {
        ...
        loop {
            ...
            loop {
                ...
                let sample_y = clamp(group_origin_y + segment_start + load_y, 0, input_dims_i.y - 1);
                tile_cols[local_id.x][u32(load_y)] = textureLoad(input_texture, vec2<i32>(output_x, sample_y), 0);
            ...
            loop {
                ...
                sum += tile_cols[local_id.x][u32(base + offset)] * weight;
                weight_total += weight;
                offset += 1;
            }
    ...
    for (var y = -radius; y <= radius; y++) {
        let sample_coord = clamp(center + vec2<i32>(0, y), vec2<i32>(0), input_dims_i - vec2<i32>(1));
        let weight = gaussian_weight(y, sigma);
