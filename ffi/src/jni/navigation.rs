...
unsafe extern "C" fn jni_navigation_apply(
    ...
    // SAFETY: `data` is the payload registered alongside this entry point by
    // `envInstallNavigationController`, kept alive until `jni_navigation_drop`.
    let data = unsafe { &*data.cast_const().cast::<JniNavigationCallbackData>() };
    data.with_env(|env| {
        let view_class = env
        ...
        env.call_method(
            ...
            &[
                JValue::Long(transaction.id.cast_signed()),
                JValue::Int(
...
unsafe extern "C" fn jni_navigation_drop(data: *mut ()) {
    // SAFETY: `data` is the boxed payload this drop entry point was registered with,
    // and the controller invokes it once.
    let _ = unsafe { Box::from_raw(data.cast::<JniNavigationCallbackData>()) };
}
...
>(
    ...
    super::with_env(&mut env, |env| {
        ...
        // SAFETY: Kotlin passes back the live app environment handle, and `data_ptr` is
        // the payload the two entry points above expect, whose ownership moves to the
        // controller.
        unsafe {
...
extern "system" fn Java_dev_waterui_android_ffi_WatcherJni_envHasNavigationController<'local>(
    ...
    // SAFETY: Kotlin passes back the live app environment handle, which this only reads.
    unsafe {
        crate::components::navigation::waterui_env_has_navigation_controller(
...
extern "system" fn Java_dev_waterui_android_ffi_WatcherJni_tabContent<'local>(
    ...
    // SAFETY: Kotlin passes back the tab-content handle the renderer owns and the live
    // app environment; `waterui_tab_content` resolves the tab against that environment.
    let nav_view = unsafe {
        crate::components::navigation::waterui_tab_content(
            content_ptr as *mut WuiTabContent,
            env_ptr as *const crate::WuiEnv,
        )
    };
    super::with_env(&mut env, |env| {
...
extern "system" fn Java_dev_waterui_android_ffi_WatcherJni_splitNavigationDetailContent<'local>(
    ...
    // SAFETY: Kotlin passes back the split-detail handle the renderer owns, the id it
    // took from the same selection binding, and the live app environment.
    let nav_view = unsafe {
        crate::components::navigation::waterui_split_navigation_detail_content(
            detail_ptr as *mut WuiNavigationSplitDetail,
