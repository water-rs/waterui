...
fn memory_safe_view() -> impl View {
    ...
    vstack(
        data.iter().map(|&item| {
            text!("Item: {}", item) // Use text! for reactive content
        }).collect::<Vec<_>>()
    )
...
fn thread_safe_shared_data() -> impl View {
    ...
    vstack((
        ...
        vstack(
            processed.iter().map(|&item| {
                text!("Processed: {}", item) // Use text! for reactive content
            }).collect::<Vec<_>>()
        ),
    ))
}
...
fn database_view() -> impl View {
    ...
    vstack((
        ...
        vstack(
            query_results.iter().map(|result| {
                text(result.clone()) // Already reactive since query_results is a signal
            }).collect::<Vec<_>>()
        ),
    ))
...
fn efficient_state_updates() -> impl View {
    ...
    let filtered_items = items.map({
        ...
        move |items| {
            items.iter()
                .filter(|item| {
                    let filter_text = filter.with(|f| f.clone());
        ...
        scroll_view(
            vstack(
                filtered_items.iter().map(|item| {
                    text(item.clone()) // Already reactive
                }).collect::<Vec<_>>()
            )
        ),
    ))
...
fn large_list_example() -> impl View {
    ...
    vstack((
        text!("Showing items {} to {} of {}", s!(visible_range.0), s!(visible_range.1), s!(items.len())),
        
        // Only render visible items to save memory
        scroll_view(
            vstack(
                s!(items[visible_range.0..visible_range.1.min(items.len())]).iter().map(|item| {
                    text(item.clone())
                }).collect::<Vec<_>>()
            )
        )
        ...
        hstack((
            button("Previous")
                .on_press({
                    let visible_range = visible_range.clone();
                    move || {
                        visible_range.update(|(start, end)| {
...
fn async_data_loader() -> impl View {
    ...
    vstack((
        ...
        when(loading_state.clone(), |loading| if loading {
            hstack((
                spinner(),
                "Loading...",
            ))
        ...
        when(error.clone(), |err| if let Some(err) = err {
            text!("Error: {}", err)
                .color(Color::red())
                .into_view()
        } else {
        ...
        scroll_view(
            vstack(
                events.iter().rev().map(|event| {
                    text(event.clone()) // Already reactive
                        .font_size(14.0)
                }).collect::<Vec<_>>()
            )
        )
...
fn memory_monitor() -> impl View {
    ...
    let (allocated, allocations) = stats.with(|s| *s);
...
fn leak_detection_demo() -> impl View {
    ...
    vstack((
        ...
        when(show_leaks.clone(), |show| if show {
            leak_report_view().into_view()
        } else {
            empty().into_view()
        }),
...
fn leak_report_view() -> impl View {
    ...
    vstack((
        ...
        when(references.clone(), |refs| if refs.is_empty() {
            text("No active references (good!)")
                .color(Color::green())
                .into_view()
        } else {
            vstack(
                refs.iter().map(|(name, count)| {
                    hstack((
                        text(name.clone()),
                        text!(":{}", count)
            ...
            .on_press({
                ...
                move || {
                    ...
                    let new_batch: Vec<String> = (0..batch_size)
                        .map(|i| format!("Item {}", s!(total + i as i32)))
                        .collect();
                    
                    batch.set(new_batch);
                    total.update(|t| t + batch_size);
                }
            }),
...
fn safe_callback_patterns() -> impl View {
    ...
    vstack((
        ...
        button(text!(if timer_active { "Stop Timer" } else { "Start Timer" }))
            .on_press({
...
fn mobile_memory_patterns() -> impl View {
    ...
    vstack((
        when(low_memory_mode.clone(), |low_memory| if low_memory {
            hstack((
        ...
        button("Add Data")
            .on_press({
                ...
                move || {
                    data.update(|data| {
                        let item_count = if low_memory.with(|lm| *lm) { 10 } else { 100 };
                        for i in 0..item_count {
                            data.push(format!("Item {}", data.len() + i));
                        }
                        
                        // Aggressively limit data in low memory mode
                        if low_memory.with(|lm| *lm) && data.len() > 500 {
                            data.truncate(500);
                        }
                    });
                }
        ...
        text!("Items: {}", s!(data.len())),
        
        // Show fewer items in low memory mode
        scroll_view(
            vstack(
                data.iter()
                    .take(if low_memory_mode.with(|lm| *lm) { 20 } else { 100 })
                    .map(|item| text(item.clone())) // Already reactive
                    .collect::<Vec<_>>()
            )
        )
        .max_height(200.0),
