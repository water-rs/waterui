...
- Hydrolysis Material 3 switches follow Material Web track treatment: unselected tracks use a 2 logical-unit outline, while selected tracks have no visible outline.
- Hydrolysis Material 3 checkboxes follow Material Web container treatment: unselected boxes use a 2 logical-unit outline, selected boxes have no outline, and the selected container/icon fade and scale in from the Material 60% start scale.
- Hydrolysis Material 3 radio indicators follow Material Web icon geometry: the icon is 20 logical units square, the outer ring is a 2 logical-unit donut, and the selected inner dot has a 5 logical-unit radius.
- Hydrolysis Material 3 sliders follow Material Web latest metrics: active and inactive tracks are 16 logical units high, inactive track uses `secondary-container`, and the handle is a 4x44 logical-unit vertical pill with a 40 logical-unit state layer.
- Hydrolysis Material 3 filled text fields follow Material Web container geometry: minimum height is 56 logical units, the filled container has 4 logical-unit top corners with square bottom corners, and the active indicator is 1 logical unit high when unfocused.
- Hydrolysis Material 3 pressed state layers use Material Web-style soft-edge radial ripple rendering: the ripple grows from the press origin toward the container center and fades to transparent at the edge instead of drawing a hard-edged M2-style circle.
- Hydrolysis Material 3 progress indicators follow Material Web v0.192 metrics: linear track and active indicator are 4 logical units high, circular progress is 48 logical units square, circular active indicator stroke width is 4 logical units, and circular progress has no visible inactive track.
- `loading()` / `Progress::infinity().circular()` renders Material-style indeterminate circular progress: the active arc rotates continuously while expanding and contracting.
