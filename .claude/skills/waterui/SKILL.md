...
- Linux GitHub Actions jobs that compile real audio/video/font backends must install native development packages for the enabled backends, including `libasound2-dev` for ALSA audio, `libva-dev` for VA-API codecs, and `libfontconfig1-dev` for Fontconfig text/font discovery. This includes `cargo hack --each-feature`, because single-feature checks can still enable those backend crates. Do not remove backend capabilities just to avoid the packages.
