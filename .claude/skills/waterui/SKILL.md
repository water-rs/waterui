...
- Linux GitHub Actions jobs that compile real all-features audio/video backends must install native development packages for the enabled backends, including `libasound2-dev` for ALSA audio and `libva-dev` for VA-API codecs. Do not remove those backend capabilities just to avoid the packages.
