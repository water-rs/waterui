...
- macOS preview dynamic linking must inject Rust stdlib rpath at build time (`rustc --print target-libdir` + `-Clink-arg=-Wl,-rpath,...`). Do not reintroduce runtime `install_name_tool` patching in support-app dylib load paths.
