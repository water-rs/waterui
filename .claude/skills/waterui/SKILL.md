...
- GitHub Actions jobs that run Cargo against the superproject must check out submodules recursively. Missing `utils/nami` or backend submodules in CI is a workflow bug, not a Rust dependency bug.
...
```text
libasound2-dev libva-dev libfontconfig1-dev libgbm-dev libxcb1-dev libglib2.0-dev libpango1.0-dev
```
