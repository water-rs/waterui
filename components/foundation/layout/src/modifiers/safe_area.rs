...
//! backend applies the insets at the container level: a stack lays its
//! children out inside the safe area and extends the scroll surfaces and
//! chrome containers that touch its edges, which inset their own content in
//! turn. Nothing `WaterUI` lays out itself — the window's snackbar and overlay
