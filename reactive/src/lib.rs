...
//!
//! A powerful, lightweight reactive state management system for building interactive user interfaces.
//!
//! ## Overview
//!
//! The WaterUI reactive framework provides a comprehensive set of primitives for managing
//! application state in a reactive way. It enables automatic propagation of changes through
//! your application, with minimal boilerplate and maximum flexibility.
//!
//! ```
//! +----------------+      +----------------+      +---------------+
...
//! ```
//!
//! Bindings serve as the source of truth for application state and notify observers
//! when their values change. They provide specialized methods for different data types:
//!
...
//! ### Compute Trait
//!
//! The `Compute` trait defines the core contract for any reactive value:
//!
//! ```rust
...
//! ```
//!
//! This trait is implemented by `Binding`, `Computed`, and all other reactive types,
//! providing a consistent interface for working with reactive values regardless of
...
//! ```
//!
//! `Computed<T>` is a type-erased container that can hold any implementation of the
//! `Compute` trait, providing a uniform way to work with different kinds of computations.
//!
//! ### Watchers
//!
...
//! ```
//!
//! When working with watchers, it's important to store the returned `WatcherGuard`.
//! This guard ensures the watcher is properly unregistered when dropped, preventing
//! memory leaks.
//!
//! ## Thread Safety Model
//!
//! The reactive framework is designed to be **single-threaded** by default. For cross-thread
//! operations, use the [`Mailbox`](mailbox/index.html) type which provides a safe bridge.
//!
//! ## Additional Features
//!
