...
#[derive(Default)]
pub(super) struct TextEditingState {
    pub(super) text_input_targets: Vec<TextInputTarget>,
    pub(super) text_selection_slots: Vec<Rc<RefCell<TextSelectionSlot>>>,
    pub(super) text_selection_cursor: usize,
    pub(super) active_text_selection_drag: Option<usize>,
    pub(super) last_text_selection_click: Option<TextSelectionClickState>,
    pub(super) active_text_context_menu: Option<ActiveTextContextMenu>,
    pub(super) focused_text_input: Cell<Option<usize>>,
    pub(super) ime_preedit: Option<Str>,
    pub(super) text_caret_fade_started_at: Option<Instant>,
    pub(super) text_caret_next_frame_at: Option<Instant>,
}
...
pub(super) struct TextSelectionClickState {
    pub(super) target_index: usize,
    pub(super) point: vello::kurbo::Point,
    pub(super) at: Instant,
    pub(super) count: u8,
}

#[derive(Clone)]
pub(super) struct TextInputTarget {
    pub(super) bounds: vello::kurbo::Rect,
    pub(super) cursor_area: vello::kurbo::Rect,
    pub(super) text_bounds: vello::kurbo::Rect,
    pub(super) layout: parley::Layout<[u8; 4]>,
    pub(super) purpose: TextInputPurpose,
    pub(super) depth: usize,
    pub(super) order: usize,
    pub(super) model: TextInputModel,
    pub(super) selection: Rc<RefCell<TextSelectionSlot>>,
...
#[derive(Clone)]
pub(super) struct TextContextMenuOverlayRow {
    pub(super) bounds: vello::kurbo::Rect,
    pub(super) entry: TextContextMenuEntry,
}

#[derive(Clone)]
pub(super) struct TextContextMenuOverlay {
    pub(super) bounds: vello::kurbo::Rect,
    pub(super) rows: Vec<TextContextMenuOverlayRow>,
    pub(super) model: TextInputModel,
    pub(super) selection: Rc<RefCell<TextSelectionSlot>>,
...
#[derive(Clone)]
pub(super) enum ActiveTextContextMenu {
    Overlay {
        index: usize,
...
pub(super) fn normalized_insert_text(inserted: &str, max_lines: Option<usize>) -> String {
    if max_lines == Some(1) {
        inserted
            .chars()
            .filter(|ch| *ch != '\n' && *ch != '\r')
...
pub(super) fn exceeds_line_limit(value: &str, max_lines: Option<usize>) -> bool {
...
pub(super) fn byte_index_to_char_offset(text: &str, index: usize) -> usize {
    let clamped = clamp_to_char_boundary(text, index);
    text[..clamped].chars().count()
}

pub(super) fn char_offset_to_byte_index(text: &str, char_offset: usize) -> usize {
    if char_offset == 0 {
        return 0;
    }
...
pub(super) fn normalized_selection_range(anchor: usize, focus: usize) -> std::ops::Range<usize> {
...
pub(super) fn delete_backward_in_selection(
    text: &mut String,
    anchor: &mut usize,
    focus: &mut usize,
) -> bool {
...
pub(super) fn delete_forward_in_selection(
    text: &mut String,
    anchor: &mut usize,
    focus: &mut usize,
) -> bool {
...
pub(super) fn selection_slot_range_for_text(
...
pub(super) fn selected_text_for_model(
...
pub(super) fn replace_model_selection(
    model: &TextInputModel,
...
pub(super) fn delete_model_backward(model: &TextInputModel, slot: &mut TextSelectionSlot) -> bool {
    let mut text = model.plain_text();
    let mut anchor = clamp_to_char_boundary(text.as_str(), slot.anchor);
    let mut focus = clamp_to_char_boundary(text.as_str(), slot.focus);
...
pub(super) fn delete_model_forward(model: &TextInputModel, slot: &mut TextSelectionSlot) -> bool {
    let mut text = model.plain_text();
    let mut anchor = clamp_to_char_boundary(text.as_str(), slot.anchor);
    let mut focus = clamp_to_char_boundary(text.as_str(), slot.focus);
...
pub(super) fn set_model_caret_position(
    model: &TextInputModel,
    slot: &mut TextSelectionSlot,
    index: usize,
) -> bool {
...
pub(super) fn move_model_caret_horizontal(
    model: &TextInputModel,
    slot: &mut TextSelectionSlot,
    backward: bool,
    extend: bool,
...
pub(super) fn spawn_clipboard_paste_task(
    model: TextInputModel,
...
pub(super) fn select_all_model_text(model: &TextInputModel, slot: &mut TextSelectionSlot) -> bool {
    let text = model.plain_text();
    if text.is_empty() {
...
pub(super) fn write_clipboard_text(text: &str) -> bool {
...
pub(super) fn selection_range_contains_index(
    model: &TextInputModel,
    slot: &TextSelectionSlot,
    index: usize,
) -> bool {
...
pub(super) fn text_context_menu_size(entries: &[TextContextMenuEntry]) -> (f64, f64) {
...
pub(super) fn text_context_menu_overlay_bounds(
...
pub(super) fn execute_text_context_menu_action(
    action: &TextContextMenuAction,
    model: &TextInputModel,
    selection: &Rc<RefCell<TextSelectionSlot>>,
    env: &Environment,
...
impl HydrolysisRenderer {
    pub(super) fn reset_text_caret_animation(&mut self, now: Instant) {
    ...
    pub(super) fn clear_text_caret_animation(&mut self) {
        self.text_editing.text_caret_fade_started_at = None;
        self.text_editing.text_caret_next_frame_at = None;
    }

    pub(super) fn advance_text_caret_animation(&mut self, now: Instant) -> bool {
    ...
    pub(super) fn set_focused_text_input(&mut self, focused: Option<usize>) -> bool {
    ...
    pub(super) fn handle_text_context_menu_overlay_pointer_down(
        &mut self,
    ...
    pub(super) fn focused_text_target_data(
        &mut self,
    ...
    pub(super) fn text_selection_index_from_point(
    ...
    pub(super) fn text_selection_range_from_point_with_click_count(
        target: &TextInputTarget,
        point: vello::kurbo::Point,
        click_count: u8,
    ) -> (usize, usize) {
    ...
    pub(super) fn next_text_selection_click_count(
        &mut self,
        target_index: usize,
        point: vello::kurbo::Point,
        at: Instant,
    ...
    pub(super) fn apply_text_selection_click_gesture(
        &mut self,
        index: usize,
        point: vello::kurbo::Point,
        click_count: u8,
    ...
    pub(super) fn update_text_selection_from_pointer(
        &mut self,
        index: usize,
        point: vello::kurbo::Point,
        extend: bool,
    ...
    pub(super) fn insert_text_into_focused_target(&mut self, text: &str) -> bool {
    ...
    pub(super) fn delete_backward_in_focused_target(&mut self) -> bool {
        let Some((_index, model, selection)) = self.focused_text_target_data() else {
            return false;
        };
        let mut slot = selection.borrow_mut();
    ...
    pub(super) fn delete_forward_in_focused_target(&mut self) -> bool {
        let Some((_index, model, selection)) = self.focused_text_target_data() else {
            return false;
        };
        let mut slot = selection.borrow_mut();
    ...
    pub(super) fn select_all_in_focused_target(&mut self) -> bool {
        let Some((_index, model, selection)) = self.focused_text_target_data() else {
            return false;
        };
        let mut slot = selection.borrow_mut();
    ...
    pub(super) fn copy_selection_in_focused_target(&mut self) -> bool {
        let Some((_index, model, selection)) = self.focused_text_target_data() else {
            return false;
        };
        if model.is_secure() {
    ...
    pub(super) fn cut_selection_in_focused_target(&mut self) -> bool {
        let Some((_index, model, selection)) = self.focused_text_target_data() else {
            return false;
        };
        if model.is_secure() {
    ...
    pub(super) fn paste_clipboard_into_focused_target(&mut self) -> bool {
        let Some((_index, model, selection)) = self.focused_text_target_data() else {
            return false;
        };
    ...
    pub(super) fn move_focused_caret_horizontal(&mut self, backward: bool, extend: bool) -> bool {
        let Some((_index, model, selection)) = self.focused_text_target_data() else {
            return false;
        };
        let mut slot = selection.borrow_mut();
    ...
    pub(super) fn move_focused_caret_to_boundary(&mut self, end: bool, extend: bool) -> bool {
        let Some((_index, model, selection)) = self.focused_text_target_data() else {
            return false;
        };
        let mut slot = selection.borrow_mut();
    ...
    pub(super) fn build_text_context_menu_entries(
        target: &TextInputTarget,
    ...
    pub(super) fn show_text_context_menu(
        &mut self,
        index: usize,
        point: vello::kurbo::Point,
        env: &Environment,
