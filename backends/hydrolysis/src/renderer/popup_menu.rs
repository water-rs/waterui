...
#[derive(Clone)]
pub(super) struct ContextMenuTarget {
    pub(super) bounds: vello::kurbo::Rect,
    pub(super) depth: usize,
    pub(super) order: usize,
    pub(super) items: nami::Computed<Vec<ResolvedMenuItem>>,
}
...
pub(super) struct PopupMenuState {
    ...
    pub(super) picker_menu_cursor: usize,
}
...
impl PopupMenuStateGroup {
    ...
    pub(super) fn push(&self, state: Binding<WindowState>) {
        self.0.borrow_mut().push(state);
    }

    pub(super) fn truncate(&self, len: usize) {
...
pub(super) fn popup_menu_size(nodes: &[PopupMenuNode]) -> (f64, f64) {
...
pub(super) fn popup_menu_window(
...
impl HydrolysisRenderer {
    pub(super) fn dismiss_active_popup_menu(&mut self) {
        if let Some(group) = self.popup_menu.active_popup_menu_group.take() {
            group.close_all();
        }
    }

    pub(super) fn topmost_context_menu_target_at_point(
        &self,
    ...
    pub(super) fn register_context_menu_target(
        &mut self,
