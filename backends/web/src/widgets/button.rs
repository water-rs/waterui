...
pub fn render_button(config: ButtonConfig, on_click: Option<Box<dyn Fn()>>) -> WebElement {
    ...
    if let Some(callback) = on_click {
        if let Some(html_element) = element.as_html_element() {
...
fn get_button_styles(variant: &ButtonVariant) -> (HashMap<&'static str, &'static str>, HashMap<&'static str, &'static str>) {
