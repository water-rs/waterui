...
pub(crate) fn draw_header_background(draw: &mut dyn DrawContext, bounds: Rect) {
    draw.fill_rect(bounds, &Brush::from(SURFACE_SUBTLE));
}

pub(crate) fn draw_cell_border(draw: &mut dyn DrawContext, bounds: Rect) {
    draw.stroke_rect(bounds, &Brush::from(OUTLINE_SUBTLE), 1.0);
}

pub(crate) fn draw_column_separator(draw: &mut dyn DrawContext, from: Point, to: Point) {
